...
// PIXEL SIZE: 100
// The variable "c" (e.g. "c<1") does not need to be updated in this script because it is still only testing the 1 combination, above.
// The other variables that define the parameter combinations to test (e.g. lower_hue) will be updated in the GAMMA section
// to reflect the above combination.
// Step 1 will be updated to run just the Gamma processing method. I will highlighted this part with my cursor to run just this selection.
// For Steps 2 & 3, the "GAMMA" section (both PART 1 and PART 2) will be highlighted together, and just that selection will be run.

/////////////////////////


// This macro involves 3 Steps:
// 1. Take the raw images and process them in (up to) 3 ways, saving those images into 3 respective folders (total of 4 folders of images).
...
//
// * Not necessary to repeat steps 2 & 3 on other processing folders sequentially (e.g. Minimum, Gamma, and then GMA images).
// * User must manually view the output csv file to see whether the combination yielded a passing accuracy score (small Avg Acc Dif).

// IMPORTANT NOTES FOR SETUP:
// 1. This macro is very similar to the one used to optimize parameters on the Train set. It is not necessary to run all possible
//    parameter combinations on the Test set, just the one that was determined to be the best combination on the Train set.
//    Update the code to run just the parameters of interest, as to save time and confusion when evaluating the performance
//    of the desired combination on the Test set. The parameters to be changed are as follows.
// 2. Which processing method is to be tested? Update Step 1 to only implement the processing method of interest. If unsure of how
//    to update the code, simply run it as-is and disregard the images generated for the unnecessary processing methods. Delete the
//    sections for the unnecessary processing methods under Steps 2 & 3, or highlight only the desired section and run the selection.
// 3. The optimization parameters are hardcoded in EACH processing section below (under "Steps 2 & 3," under each processing section
//    like "RAW," under each "PART 1," at the top of the initial set of for loops AND lower down at the "ANALYZE PARTICLES" for loop).
//    Manually adjust these to loop through just the parameter options of interest. The settings that are currently hardcoded are:
...
// 4. The number of optimization parameter options should be 1 for each (1 threshold, 1 pixel size, etc.), meaning only one
//    combination will be produced. This number is used in Step 3 (aka "PART 2") and would need to be updated if the parameter options
//    differ from 1 each. The number of combinations is # thresholds * # lower hues * # upper hues * # pixel size options. Adjust "c<1" here:
//    for (c=0; c<1; c++) { //NUMBER OF COMBINATIONS: 5 thresh * 5 lower hue * 3 upper hue * 25 pixel sizes = 1875.
// 5. Update the folder paths if needed. The two types of inputs to this code are merged images and cell markers.
...
//    "D:/Projects/ARC_Auto_Count_Project/TestSet/1_Markers/"
//    The filenames for the markers should start with the entire image filename, before the "_M" (aka the "subject_id" variable).
// 6. Running the optimization section (Steps 2 & 3) requires several csv files to be made ahead of time. For Step 2 (aka "PART 1"),
//    one csv file PER IMAGE must be stored in the "Optimizations" subfolder nested within the desired processing folder (e.g. "3_MergeRaw").
...
//    For Step 3 (aka "PART 2"), create one csv file to be stored in the desired processing folder's nested "Best_Parameter_Combinations"
//    subfolder. This file should be named "Optimization_Avgs.csv" and have the following text in the first cell: Avg Acc
// 7. If errors are encountered in the "PART 2" sections, some potential solutions are documented as comments under the "RAW" section.
// 8. If you encounter problems, try running each section separately (highlight the text and run just that selection).
...
proj_folder = "D:/Projects/ARC_Auto_Count_Project_EXAMPLE_COMPLETE/TestSet/";
...
merge_folder = "D:/Projects/ARC_Auto_Count_Project_EXAMPLE_COMPLETE/TestSet/3_MergeRaw/";

// Can only compare to one set of manual counts. Edit folder path appropriately.
// Manual count markers are here:
manual_count_folder = "D:/Projects/ARC_Auto_Count_Project_EXAMPLE_COMPLETE/TestSet/1_Markers/";
...
// Main merged files to be automatically counted are here:
merge_folder = "D:/Projects/ARC_Auto_Count_Project_EXAMPLE_COMPLETE/TestSet/3_MergeGamma/";
...
for (f=0; f < files.length; f++) {
	if (endsWith(files[f], ".tif")) {
		// SET THE PARAMETERS TO LOOP THROUGH: THRESHOLD, LOWER HUE LIMIT, UPPER HUE LIMIT
		for (t=0; t < thresholds.length; t++){
			for (lower_hue = 58; lower_hue < 59; lower_hue=lower_hue+6) { // 5 options, range of 50-74, up by 6's
				for (upper_hue = 113; upper_hue < 114; upper_hue=upper_hue+2) { // 3 options, range of 109-113, up by 2's
...
////////////////////////////////////////////////////////////////////
					...
					// SET THE PIXEL SIZE PARAMETER TO LOOP THROUGH
					for (cell_pixel_size = 100; cell_pixel_size < 101; cell_pixel_size = cell_pixel_size + 10) { // 25 options, range of 40-280, up by 10's
						selectWindow("Result of Result of 0");
						run("Analyze Particles...", "size="+cell_pixel_size+"-Infinity pixel show=Outlines exclude add");
						selectWindow("Drawing of Result of Result of 0");
						nROIs = roiManager("count"); // number of objects
...
// Main merged files to be automatically counted are here:
merge_folder = "D:/Projects/ARC_Auto_Count_Project_EXAMPLE_COMPLETE/TestSet/3_MergeMinimum/";
...
// Main merged files to be automatically counted are here:
merge_folder = "D:/Projects/ARC_Auto_Count_Project_EXAMPLE_COMPLETE/TestSet/3_MergeGMA/";
